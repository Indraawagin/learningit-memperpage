...
import { statusCourses, messageCourses, watchCourses } from "src/store/actions/courses";

import SidebarClass from "src/parts/SidebarClass";
import Loading from "src/parts/Loading";
import Centered from "src/parts/Centered";
...
function DetailsClass({ match, history }) {
  ...
  if (COURSES.status === "ok" && COURSES?.data?.[match.params.class]?.chapters) {
    ...
    currentLesson =
      currentChapter?.lessons?.find((lesson) => lesson.video === match.params.uid) ??
