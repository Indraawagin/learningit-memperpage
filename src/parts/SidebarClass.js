...
import { ReactComponent as ArrowBack } from "src/assets/images/icon-arrow-back.svg";
...
function SidebarClass({ data, match, defaultUri }) {
          ...
          <li key={`${chapter.course_id}-${lesson.id}-${index2}`}>
            <Link
              className={[
                ...
                getNavLinkClass(`/courses/${data.id}/${chapter.id}/${lesson.video}`),
              ].join(" ")}
  ...
  return (
    <>
      ...
      <aside
        className="bg-blue-900 max-h-screen h-screen overflow-y-auto transition-all duration-300 min-h-full fixed sm:relative z-50"
        style={sidebarStyle}
      >
        {toggleMenu && (
        ...
        <div
          className="max-h-screen h-screen fixed bg-blue-900 flex flex-col content-between z-10"
          style={{ width: 280 }}
        >
